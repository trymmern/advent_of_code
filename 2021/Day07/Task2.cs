...
namespace AdventOfCode.Day07
{
    ...
    public static class Task2
    {
        ...
        {
            var positions = CommonHelpers.ReadInstructions("Day07\\input.txt")[0].Split(',').Select(int.Parse).ToList();
            var min = positions.Min();
            var max = positions.Max() + 1;
            var costs = new List<int>();

            Console.WriteLine($"Min pos: {min}\nMax pos: {max}");
            
            for (var i = min; i < max; i++)
            {
                costs.Add(positions.Sum(j => GetCost(j, i)));
            }
        ...
        {
            var n = Math.Abs(a - b);
            return (int)Math.Floor(n * (n + 1) / (double)2);
        }
    }
}
