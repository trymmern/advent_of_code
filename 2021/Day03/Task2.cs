...
namespace AdventOfCode.Day03
{
    ...
    /// 10110 and 10111.
    /// In the fifth position, there are an equal number of 0 bits and 1 bits(one each). So, to find the oxygen generator rating, keep the number with a 1 in that position:
    /// 10111.
    ...
    public static class Task2
    {
        ...
        {
            const int bits = 12;
            var binaryStrings = CommonHelpers.ReadInstructions("Day03\\input.txt");

            var potO2GeneratorRatings = new List<string>();
            potO2GeneratorRatings.AddRange(binaryStrings);
            var potCo2ScrubRatings = new List<string>();
            potCo2ScrubRatings.AddRange(binaryStrings);

            for (var b = 0; b < bits; b++)
            {
                var mostCommonBitAtIndex = 
                    Task1.GetMostOrLeastCommonBitValueAtIndex(potO2GeneratorRatings, b, true, Task1.PreferredBit.ONE);
                Console.WriteLine($"Most common bit at index {b} is {mostCommonBitAtIndex}");
                
                var leastCommonBitAtIndex =
                    Task1.GetMostOrLeastCommonBitValueAtIndex(potCo2ScrubRatings, b, false, Task1.PreferredBit.ZERO);
                Console.WriteLine($"Least common bit at index {b} is {leastCommonBitAtIndex}");

                if (potO2GeneratorRatings.Count != 1)
                {
                    potO2GeneratorRatings =
                        potO2GeneratorRatings.Where(x => x.Substring(b, 1) == mostCommonBitAtIndex).ToList();
                }

                if (potCo2ScrubRatings.Count != 1)
                {
                    potCo2ScrubRatings = 
                        potCo2ScrubRatings.Where(x => x.Substring(b, 1) == leastCommonBitAtIndex).ToList();
                }

                Console.WriteLine("\n\nPotential oxygen generator ratings:");
                foreach (var x in potO2GeneratorRatings)
                {
                    Console.WriteLine($"{x}");
                }

                Console.WriteLine("\n\nPotential CO2 scrubber ratings:");
                foreach (var x in potCo2ScrubRatings)
                {
                    Console.WriteLine($"{x}");
                }
            }

            var o2GeneratorRating = Convert.ToInt32(potO2GeneratorRatings.First(), 2);
            var co2ScrubRating = Convert.ToInt32(potCo2ScrubRatings.First(), 2);

            return o2GeneratorRating * co2ScrubRating;
        }
    }
}
