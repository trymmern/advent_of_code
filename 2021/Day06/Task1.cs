...
namespace AdventOfCode.Day06
{
    ...
    {
        public static int GetLanternfishPopulation(int days)
        {
            var population = CommonHelpers.ReadInstructions("Day06\\input.txt")[0].Split(',').Select(int.Parse).ToList();

            Console.Write("\nInitial state: ");
            population.ForEach(f => Console.Write($"{f}, "));

            for (var i = 0; i < days; i++)
            {
                SpawnFish(population);
            }

            return population.Count;
        }

        private static void SpawnFish(List<int> population)
        {
