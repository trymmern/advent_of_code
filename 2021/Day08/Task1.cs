...
namespace AdventOfCode.Day08
{
    ...
    /// Because 7 is the only digit that uses three segments, dab in the above example means that to render a 7, signal lines d, a,
    /// and b are on. Because 4 is the only digit that uses four segments, eafb means that to render a 4, signal lines e, a, f, and b are on.
    /// 
    ...
    {
        public static int GetUniqueDigitCount()
        {
            var lines = CommonHelpers.ReadInstructions("Day08\\input.txt");
            var rawOutput = lines.Select(x => x.Split('|')[1]).ToList();
            var uniqueLengths = new List<int> { 2, 4, 3, 7 };

            var outputs = new List<string>();
            foreach (var i in rawOutput)
            {
                outputs.AddRange(i.Split(' ').Where(s => !string.IsNullOrEmpty(s)));
            }

            return outputs.Count(output => uniqueLengths.Any(x => x == output.Length));
        }
    }
}
