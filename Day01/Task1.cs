...
namespace AdventOfCode.Day01
{
    ...
    ///As the submarine drops below the surface of the ocean, it automatically performs a sonar sweep of the nearby sea floor.On a small screen, the sonar sweep report (your puzzle input) appears: each line is 
    ...
    ///
    ///The first order of business is to figure out how quickly the depth increases, just so you know what you're dealing with - you never know if the keys will get carried into deeper water by an ocean current 
    ...
    public static class Task1
    {
        ...
        private static int GetIncreasedDepthValueCount(List<string> values)
        {
            ...
            for (var i = 1; i < values.Count; i++)
            {
                ...
                {
                    Console.WriteLine($"{values[i]} > {values[i-1]} (Increased)");
                    count++;
                }
                else if (int.Parse(values[i]) == int.Parse(values[i - 1]))
                {
                    Console.WriteLine($"{values[i]} == {values[i - 1]} (No change)");
                }
                else
                {
                    Console.WriteLine($"{values[i]} < {values[i - 1]} (Decreased)");
                }
            }
            return count;
        }
